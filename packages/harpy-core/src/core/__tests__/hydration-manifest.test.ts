...
describe('Hydration Manifest', () => {
  ...
  describe('file operations', () => {
    ...
    it('should deserialize manifest from JSON string', () => {
      const mockData = JSON.stringify({
        components: [{ name: 'LoadedComponent', path: '/Comp.tsx', isClient: true }]
      });
  ...
  describe('deduplication', () => {
    it('should prevent duplicate component entries', () => {
      ...
      const unique = Array.from(new Set(components.map(c => c.name)))
    ...
    it('should allow same name in different paths', () => {
      ...
      const uniqueByPath = Array.from(new Set(components.map(c => c.path)))
